...
class HaveViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        helper.getAllBooks(for: BookCategorySaving.have.rawValue)
...
extension HaveViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if helper.models.count == 0 {
            tableView.setEmptyView(title: "Nada por aqui",
                                   message: "Vá em Pesquisa para salvar livros na sua biblioteca.")
        } else {
            tableView.restore()
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let volume = MyGoogleBooks.VolumeInfo(title: filteredBooks?[indexPath.row].title,
                                              ...
                                              description: filteredBooks?[indexPath.row].bookDescription)
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") {
            ...
            self.helper.getAllBooks(for: BookCategorySaving.have.rawValue)
